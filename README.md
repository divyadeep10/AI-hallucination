...
- **Backend** (`backend/`): FastAPI, PostgreSQL (Alembic), Redis/RQ, LLM (OpenAI or Gemini), agents (Planner → Generator → Claim Extractor → Retriever → Verifier → **External Retrieval** → Critic → Refiner), evaluation API and metrics. Permanent second retrieval: internal first, then Playwright (web search); both evidence layers combined. Enable/disable with `EXTERNAL_RETRIEVAL_ENABLED` in .env. See [docs/EXTERNAL_RETRIEVAL.md](docs/EXTERNAL_RETRIEVAL.md).
